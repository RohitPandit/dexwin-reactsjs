...
export default function (props) {
  let { gameid, homeImg, awayImg } = props;
  const [threeWayOdds, setThreeWayOdds] = useState([]);
  var status = false;

  useEffect(() => {
    getThreeWayOddAPIOddUsignGameId(gameid);
  }, []);

  const getThreeWayOddAPIOddUsignGameId = async (gameid) => {
    try {
      const endPoint = "threeWayOddAPI";
      const response = await getOddAPI(endPoint, gameid);
      status = true;
